...
class CopyEncoder(nn.Module):
	def __init__(self, vocab_size, embed_size, hidden_size, embeddings_array):
		super(CopyEncoder, self).__init__()

		self.embed = nn.Embedding.from_pretrained(embeddings_array)
...
class CopyDecoder(nn.Module):
	...
	def forward(self, input_idx, encoded, encoded_idx, prev_state, weighted, order):
		...
		#print("State before squeezing : ", state.shape)
		if state.shape[1] != 1:
